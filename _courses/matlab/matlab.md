...
| `sqrt(x)` | 開根號函數，計算√x  |
| `nthroot(x,n)` | 開根號函數，計算n√x  |

.

| **三角函數** | **說明** |
| --- | --- |
| `sin, cos, tan, cot, sec, csc`   | 三角函數 (單位為弳度)  |
| `asin, acos, atan, acot, asec, acsc` | 反三角函數 (單位為弳度) |
| `sind, cosd, tand, cotd, secd, cscd` | 三角函數 (單位為度) |
| `asind, acosd, atand, acotd, asecd, acscd` | 反三角函數 (單位為度) |
